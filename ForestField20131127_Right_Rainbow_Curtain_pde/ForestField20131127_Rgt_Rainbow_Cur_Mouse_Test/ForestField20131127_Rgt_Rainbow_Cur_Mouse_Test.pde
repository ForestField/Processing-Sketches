...
float rssi;// signal strength received from the the transmitting dancer's 
...
float[] readings = new float[window_size]; //create an array called "readings" that will be averaged
float index = 0;//start at the first index of the array
float total = 0;//total will = total- readings[index]
float average = 0; //average will = total/window_size
float low_rssi = 36;//lowest rssi value->maps to 1
float high_rssi = 80;// highest rssi value-> maps to width
PFont font;
//Audio Variables:
...
for (int i=0;i>width/2-50 && i<width/2-25;i+=25) {
    for (int j=0;j<height;j+=25) {
  ...
  for (int i=width;i>width/2;i-=25) {
    for (int j=0;j<height;j+=25) {
      ...
      strokeWeight(20);
      stroke(random(width-i+diff),random(width-i+diff),random(width-i-diff));
      beginShape();
      vertex(i, j);
      vertex(i-100, average);
      endShape();
