  ...
  for (int i=0;i<width;i+=25) {
    for (int j=0;j<height;j+=25) {
      if (i<width/2) {
        strokeWeight(20);
                 if (closeCount==0){
                  float greyStroke=map(random(i-diff, j-diff),i-diff,j-diff,128,192);
                   stroke(greyStroke,greyStroke,greyStroke);
                 }
                 else{stroke(random(width-i+diff), random(width-i+diff), random(width-i+diff));}
        beginShape();
        vertex(i, j);
