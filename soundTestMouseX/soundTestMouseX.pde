...
void draw()
{
  ...
  float val=map(mouseX,0,screenWidth,34,90);
   background(255-r, 255-g, 255-b);
...
freq=freq0;
 float test=random(1);
  ...
  println(test);
  if(soundState!=soundState){;}
  else if (soundState==soundState){
  delay(500);}}
...
else if (val>=68 && val<=80){
//
soundState=6;
freq4=27.5+27.5*3/2+27.5*3/2+27.5/3*2;
freq=freq4;
 float test=random(1);
   println(test);
      if (test<.5){test=0;}
...
//
soundState=7;
freq5=27.5+27.5*3/2+27.5*3/2+27.5*3/2+27.5/3*2;
freq=freq5;
 float test=random(1);
   println(test);
      if (test<.5){test=0;}
