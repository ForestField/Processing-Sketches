...
public class Fractal_NYshow extends PApplet {
...
float offIncrement = .7f;//amount volume is decresead per away time step
float volMax = 120.0f;//maximum volume
float volMin = 30.0f;//minimum volume
...
//*************SET UP THE SKETCH*********\\
public void setup(){
   size(windowWidth, windowHeight);//set the size to 1100 px wide and 768 high
...
//**MAIN HEADER-------------AUDIO------------**\\
  ...
  sineG = new SineWave(196.00f, off, out.sampleRate());//1st sine wave - frequency between G and an A
  sineC = new SineWave(130.81f, off, out.sampleRate());//2nd sine wave - frequency between C and D
  sineF = new SineWave(174.61f, off, out.sampleRate());//3rd sine wave - frequency ~F
  sineB = new SineWave(20.60f, off, out.sampleRate()); 
...
public void branch(float h) {
  ...
  if (rssi<=36)//if the rssi is less than 36
    c=random(.6f, .62f);//slightly randomize the number of branches 
...
// Draw the lines. If the pixel size is smaller than 2, stop the recursive branching
  ...
  static public void main(String args[]) {
    PApplet.main(new String[] { "--present", "--bgcolor=#666666", "--hide-stop", "Fractal_NYshow" });
  }
}
